...
class NASBench201API(object):
    ...
    def get_more_info(
        ...
        if dataset == "cifar10-valid":
            ...
            try:
                test__info = archresult.get_metrics(
            ...
            xifo = {
                ...
                "train-per-time": None
                if train_info["all_time"] is None
                else train_info["all_time"] / total,
                "train-all-time": train_info["all_time"],
                "valid-loss": valid_info["loss"],
                "valid-accuracy": valid_info["accuracy"],
                "valid-all-time": valid_info["all_time"],
                "valid-per-time": None
                if valid_info["all_time"] is None
                else valid_info["all_time"] / total,
            }
            ...
            try:
                ...
                valid_info = None
            try:
                est_valid_info = archresult.get_metrics(
                    dataset, "ori-test", iepoch=iepoch, is_random=is_random
                )
            except:
                est_valid_info = None

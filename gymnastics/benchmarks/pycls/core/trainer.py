...
import numpy as np
import gymnastics.benchmarks.pycls.core.benchmark as benchmark
import gymnastics.benchmarks.pycls.core.builders as builders
import gymnastics.benchmarks.pycls.core.checkpoint as checkpoint
import gymnastics.benchmarks.pycls.core.config as config
import gymnastics.benchmarks.pycls.core.distributed as dist
import gymnastics.benchmarks.pycls.core.logging as logging
import gymnastics.benchmarks.pycls.core.meters as meters
import gymnastics.benchmarks.pycls.core.net as net
import gymnastics.benchmarks.pycls.core.optimizer as optim
import gymnastics.benchmarks.pycls.datasets.loader as loader
import torch
...
def train_epoch(train_loader, model, loss_fun, optimizer, train_meter, cur_epoch):
    ...
    for cur_iter, (inputs, labels) in enumerate(train_loader):
        ...
        if isinstance(preds, tuple):
            loss = loss_fun(preds[0], labels) + cfg.NAS.AUX_WEIGHT * loss_fun(
                preds[1], labels
            )
            preds = preds[0]
        else:
        ...
        if cfg.TASK == "seg":
            ...
            top1_err, top5_err = meters.inter_union(
                preds, labels, cfg.MODEL.NUM_CLASSES
            )
        else:

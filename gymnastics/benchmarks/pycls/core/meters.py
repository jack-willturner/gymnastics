...
from gymnastics.benchmarks.pycls.core.timer import Timer
...
def inter_union(preds, labels, num_classes):
    ...
    area_inter = torch.histc(
        inter.type(torch.int64), bins=num_classes, min=1, max=num_classes
    )
    area_preds = torch.histc(
        preds.type(torch.int64), bins=num_classes, min=1, max=num_classes
    )
    area_labels = torch.histc(
        labels.type(torch.int64), bins=num_classes, min=1, max=num_classes
    )
    area_union = area_preds + area_labels - area_inter

    return [
        area_inter.type(torch.float64) / labels.size(0),
        area_union.type(torch.float64) / labels.size(0),
    ]

...
def remap_dataset_set_names(dataset, metric_on_set, verbose=False):
    ...
    if verbose:
        print(
            "Call internal function _remap_dataset_set_names with dataset={:} and metric_on_set={:}".format(
...
class NASBenchMetaAPI(metaclass=abc.ABCMeta):
    ...
    def __init__(
        self,
        file_path_or_dict: Optional[Union[Text, Dict]] = None,
        verbose: bool = True,
    ):
    ...
    def simulate_train_eval(
        ...
        assert dataset in all_names, "Invalid dataset name : {:} vs {:}".format(
            dataset, all_names
        )
        if dataset == "cifar10":
    ...
    def clear_params(self, index: int, hp: Optional[Text] = None):
        ...
        if hp is None:
            ...
            if str(hp) not in self.arch2infos_dict[index]:
                raise ValueError(
                    "The {:}-th architecture only has hyper-parameters of {:} instead of {:}.".format(
    ...
    def _query_info_str_by_arch(self, arch, hp: Text = "12", print_information=None):
        ...
        if arch_index in self.arch2infos_dict:
            if hp not in self.arch2infos_dict[arch_index]:
                raise ValueError(
                    "The {:}-th architecture only has hyper-parameters of {:} instead of {:}.".format(
                        index, list(self.arch2infos_dict[arch_index].keys()), hp
                    )
                )
    ...
    def query_by_index(
        ...
        if self.verbose:
            print(
                "Call query_by_index with arch_index={:}, dataname={:}, hp={:}".format(
                    arch_index, dataname, hp
                )
            )
    ...
    def find_best(
        self, dataset, metric_on_set, FLOP_max=None, Param_max=None, hp: Text = "12"
    ):
        """Find the architecture with the highest accuracy based on some constraints."""
        if self.verbose:
            print(
                "Call find_best with dataset={:}, metric_on_set={:}, hp={:} | with #FLOPs < {:} and #Params < {:}".format(
                    dataset, metric_on_set, hp, FLOP_max, Param_max
                )
            )
    ...
    def get_net_param(self, index, dataset, seed: Optional[int], hp: Text = "12"):
        ...
        if self.verbose:
            print(
                "Call the get_net_param function with index={:}, dataset={:}, seed={:}, hp={:}".format(
                    index, dataset, seed, hp
                )
            )
    ...
    def get_net_config(self, index: int, dataset: Text):
        ...
        if self.verbose:
            print(
                "Call the get_net_config function with index={:}, dataset={:}.".format(
                    index, dataset
                )
            )
        if index in self.arch2infos_dict:
            ...
            raise ValueError(
                "The arch_index={:} is not in arch2infos_dict.".format(arch_index)
            )
    ...
    def get_cost_info(
        ...
        if self.verbose:
            print(
                "Call the get_cost_info function with index={:}, dataset={:}, and hp={:}.".format(
                    index, dataset, hp
                )
            )
        info = self.query_meta_info_by_index(index, hp)
    ...
    def get_latency(self, index: int, dataset: Text, hp: Text = "12") -> float:
        ...
        if self.verbose:
            print(
                "Call the get_latency function with index={:}, dataset={:}, and hp={:}.".format(
                    index, dataset, hp
                )
            )

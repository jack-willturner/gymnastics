...
class NASBench301API(NASBenchMetaAPI):
    ...
    def __init__(
        ...
        if file_path_or_dict is None:
            file_path_or_dict = os.path.join(
                os.environ["TORCH_HOME"], ALL_BENCHMARK_FILES[-1]
            )
            print(
                "Try to use the default NAS-Bench-301 path from {:}.".format(
                    file_path_or_dict
                )
            )
        ...
        if self.verbose:
            print(
                "Create NAS-Bench-301 done with {:}/{:} architectures avaliable.".format(
                    len(self.evaluated_indexes), len(self.meta_archs)
                )
            )
    ...
    def query_info_str_by_arch(self, arch, hp: Text = "12"):
        ...
        if self.verbose:
            print(
                "Call query_info_str_by_arch with arch={:} and hp={:}".format(arch, hp)
            )

from gymnastics.benchmarks.datasets import get_datasets
...
def get_data(
    dataset, data_loc, trainval, batch_size, augtype, repeat, args, pin_memory=True
):
    train_data, valid_data, xshape, class_num = get_datasets(
        dataset, data_loc, cutout=0
    )
    if augtype == "gaussnoise":
        ...
        train_data.transform.transforms = train_data.transform.transforms[2:]
        train_data.transform.transforms.append(
            torchvision.transforms.RandomErasing(p=0.9, scale=(0.02, 0.04))
        )
    elif augtype == "none":
        train_data.transform.transforms = train_data.transform.transforms[2:]
        ...
        if repeat > 0:
            train_loader = torch.utils.data.DataLoader(
                ...
                sampler=RepeatSampler(
                    torch.utils.data.sampler.SubsetRandomSampler(train_split), repeat
                ),
            )
        else:
            train_loader = torch.utils.data.DataLoader(

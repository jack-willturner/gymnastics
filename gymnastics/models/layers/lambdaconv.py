...
class LambdaConv(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.lambda_conv = nn.Conv3d(
            du, dk, (1, r, r), padding=(0, (r - 1) // 2, (r - 1) // 2)
        )
    ...
    def forward(self, x):
        ...
        qkv = self.conv_qkv(x)
        q, k, v = torch.split(
            qkv, [self.Nh * self.dk, self.dk * self.du, self.dv * self.du], dim=1
        )
        ...
        lambda_p = self.lambda_conv(v.view(N, self.du, self.dv, H, W)).view(
            N, self.dk, self.dv, H * W
        )
